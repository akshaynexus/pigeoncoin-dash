...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
	...
	if(pblock->txoutFounder != CTxOut()) {
		founderObj.push_back(Pair("payee", founderPayment.GetFounderPayeeAddr(pindexPrev->nHeight + 1)));
	...
	result.push_back(Pair("founder_payments_started", founderPayment.shouldPayFounder(pindexPrev->nHeight + 1)));
    return result;
}
